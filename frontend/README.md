...
排除/恢复请求封装仍保留。导航将原件管理与逻辑知识库管理分开；知识库页支持 CRUD、成员分页、
批量添加和批量移除，同一论文可加入多个知识库。
...
`originalsPath`。Chat 可选择全部可检索论文、一个知识库或一篇 `searchable=true` 的论文；
空知识库不能提交，切换范围会创建新会话。浏览器不提供原件硬删除。完整三端验收状态见
...
| `src/chat/` | 问答流状态与状态转换 |
| `src/pages/` | 论文原件页、逻辑知识库页、问答页及页面测试 |
