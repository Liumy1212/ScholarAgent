...
论文 RAG 闭环，并在三端实现本地论文原件库、逻辑知识库、扫描、筛选、手动入库和知识删除能力。
阶段 1.4 的原件库基线与日常操作修复曾通过合成 PDF 全栈冒烟及三端模块检查；最近完成的
论文目录扁平化已通过契约与 Agent 检查，但受本机 Docker 引擎状态限制，尚待补做真实数据库
迁移后的扫描与全栈复验。当前重点是阶段 1.5 的日常使用加固，详细状态见路线图。
...
- Python 在 `.private/paper-library/` 管理 PDF 原件；网页上传保存到
  同一目录，递归目录结构仅用于原件整理，与逻辑知识库成员关系彼此独立。
- 可创建、重命名、删除逻辑知识库并批量维护成员；同一论文可属于多个知识库，PDF、chunk
  和向量始终只保存一份。
- Agent 与 Java 提供原件上传、服务端状态筛选、目录扫描、手动入库、知识删除、兼容性
  排除/恢复和 PDF Range 接口。
- React 展示实际扫描目录、统一原件清单、筛选后的总数、入库进度和可检索状态，并提供
  最多 10 篇 PDF 的串行上传/入库、仅上传原件、逐篇入库、失败重试、PDF 预览与知识删除。
  批量导入由浏览器逐篇显式调用上传与入库接口；上传和扫描接口本身不会自动入库。
- Chat 支持全部可检索论文、一个知识库或单篇论文三种范围；知识库范围在建流前生成成员快照。
...
功能、契约、运行配置或部署流程发生变化时，必须在同一项工作中同步更新对应的唯一事实来源；
具体维护规则见 [AI 仓库执行规则](AGENTS.md) 与各目录下最近的 `AGENTS.md`。
